...
class Backtester:
    ...
    def fetch_historical_data(self) -> pd.DataFrame:
        ...
        try:
                ...
                if isinstance(data.columns, pd.MultiIndex):
                    ...
                    try:
                        self.price_data = data[
                            ("Adj Close", self.symbols[0])
                        ].to_frame()
                        self.price_data.columns = [self.symbols[0]]
                    except KeyError:
                        try:
                            self.price_data = data[
                                ("Close", self.symbols[0])
                            ].to_frame()
                            self.price_data.columns = [self.symbols[0]]
                        except KeyError:
                            ...
                            price_cols = [
                                ...
                                if col[0] in ["Adj Close", "Close", "Open"]
                            ]
                            if price_cols:
                    ...
                    try:
                        ...
                        try:
                            ...
                            available_price_types = [
                                level
                                for level in data.columns.levels[0]
                                if level
                                in ["Adj Close", "Close", "Open", "High", "Low"]
                            ]
            ...
            if invalid_symbols:
                print(
                    f"Warning: Removing symbols with insufficient data: {invalid_symbols}"
                )

                # Recalculate weights for valid symbols only
                valid_weights = {
                    s: self.weights[s] for s in valid_symbols if s in self.weights
                }
                ...
                if total_valid_weight > 0:
                    # Normalize weights
                    self.weights = {
                        s: w / total_valid_weight for s, w in valid_weights.items()
                    }
            ...
            self.price_data = self.price_data.fillna(method="ffill").fillna(
                method="bfill"
            )
            ...
            print(
                f"Date range: {self.price_data.index[0].date()} to {self.price_data.index[-1].date()}"
            )
    ...
    def calculate_portfolio_value(self) -> pd.DataFrame:
        ...
        try:
            print(
                f"Calculating portfolio performance with {self.rebalance_frequency} rebalancing..."
            )
            ...
            for i, (date, _) in enumerate(results.iterrows()):
                ...
                if is_rebalance_day or i == 0:
                    ...
                    if i > 0:
                        portfolio_value = (
                            sum(
                                ...
                                * self.price_data.loc[date, symbol]
                                for symbol in self.symbols
                            )
                            + current_cash
                        )
                    ...
                    # Rebalance portfolio
                    target_values = {
                        symbol: portfolio_value * self.weights[symbol]
                        for symbol in self.symbols
                    }
            ...
            results["cumulative_return"] = (
                results["total_value"] / self.initial_investment
            ) - 1

            self.portfolio_values = results
            print(
                f"Portfolio calculation complete. Final value: ${results['total_value'].iloc[-1]:,.2f}"
            )
    ...
    def calculate_metrics(self) -> Dict:
        ...
        try:
            ...
            var_95 = (
                np.percentile(daily_returns.dropna(), 5)
            ...
            calmar_ratio = (
                abs(annualized_return / max_drawdown) if max_drawdown < 0 else 0
            )
        ...
        try:
            ...
            report = {
                ...
                "performance_metrics": {
                    "returns": {
                        "total_return": self.metrics["total_return_percentage"],
                        "annualized_return": self.metrics[
            ...
            if include_detailed_data and self.portfolio_values is not None:
                report["daily_data"] = {
                    ...
                    "returns": self.portfolio_values["portfolio_return"]
                    .fillna(0)
                    .tolist(),
                    "cumulative_returns": self.portfolio_values[
                        "cumulative_return"
                    ].tolist(),
                }
                ...
                for symbol in self.symbols:
                    report["daily_data"][f"{symbol}_values"] = self.portfolio_values[
                        f"{symbol}_value"
                    ].tolist()
