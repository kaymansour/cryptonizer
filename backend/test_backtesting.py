...
def test_strategy_comparison():
    ...
    try:
        ...
        assert 'comparison_summary' in results, "Results should contain comparison summary"
...
def test_edge_cases():
    ...
    try:
        ...
        print(f"✅ Single symbol backtest worked: Final value ${report['summary']['final_value']:,.2f}")
        passed_tests += 1
...
def test_api_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Backtest API: Final value ${data['summary']['final_value']:,.2f}")
            passed_tests += 1
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ Strategy Comparison API: {data['summary']['strategies_compared']} strategies compared")
            passed_tests += 1
        else:
            print(f"❌ Strategy Comparison API failed: {response.status_code} - {response.text}")
...
def test_rebalancing_frequencies():
    ...
    for freq in frequencies:
        try:
            ...
            print(f"  ✅ {freq}: Final value ${final_value:,.2f}, Return {total_return:.2f}%")
    ...
    for freq, data in results.items():
        print(f"  {freq:8}: ${data['final_value']:8,.0f} ({data['total_return']:6.2f}%)")
