...
class IntradayPredictor:
    ...
    def __init__(
        ...
        self.model_path = f"models/{symbol}_{interval}_predictor.h5"
        self.scaler_path = f"models/{symbol}_{interval}_scaler.pkl"

    def fetch_intraday_data(
        self, days_back: int = 730
    ) -> pd.DataFrame:
        """
    ...
    def prepare_training_data(
        ...
        # Create sequences
        X, y = [], []
        for i in range(self.lookback_periods, len(scaled_data) - self.prediction_horizon):
    ...
    def predict_next(self, recent_data: pd.DataFrame) -> Dict:
        ...
        if len(recent_data) < self.lookback_periods:
            raise ValueError(f"Need at least {self.lookback_periods} periods of data")

        # Prepare input
        feature_columns = ['Close', 'Volume', 'MA_20', 'MA_50', 'RSI', 'Price_Change', 'Volatility']
        features = recent_data[feature_columns].tail(self.lookback_periods).values
        scaled = self.scaler.transform(features)
        X = scaled.reshape(1, self.lookback_periods, len(feature_columns))
...
if __name__ == "__main__":
    ...
    for symbol, result in results.items():
        if 'error' not in result:
            print(f"{symbol}: MSE={result['test_loss']:.6f}, MAE={result['test_mae']:.6f}")
        else:
