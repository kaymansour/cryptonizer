...
def compare_models(
    symbol: str, interval: str = "4h", epochs: int = 50, batch_size: int = 32
) -> Dict:
    """
    ...
    mse_improvement = (
        (vanilla_results["test_loss"] - attention_results["test_loss"])
        / vanilla_results["test_loss"]
    ) * 100
    mae_improvement = (
        (vanilla_results["test_mae"] - attention_results["test_mae"])
        / vanilla_results["test_mae"]
    ) * 100
    ...
    print("-" * 70)
    print(
        f"{'Test Loss (MSE)':<20} {vanilla_results['test_loss']:>15.6f} {attention_results['test_loss']:>15.6f} {mse_improvement:>14.2f}%"
    )
    print(
        f"{'Test MAE':<20} {vanilla_results['test_mae']:>15.6f} {attention_results['test_mae']:>15.6f} {mae_improvement:>14.2f}%"
    )
    print(
        f"{'Training Samples':<20} {vanilla_results['training_samples']:>15} {attention_results['training_samples']:>15} {'Same':>15}"
    )
    print(
        f"{'Test Samples':<20} {vanilla_results['test_samples']:>15} {attention_results['test_samples']:>15} {'Same':>15}"
    )
    print("=" * 70)
...
def batch_compare_models(
    symbols: List[str], interval: str = "4h", epochs: int = 50, batch_size: int = 32
) -> Dict:
    """
    ...
    for symbol, result in results.items():
        if "error" not in result:
            ...
            print(
                f"{icon} {symbol:<15} Winner: {winner.upper():<10} MSE Change: {mse_improvement:>7.2f}%"
            )
...
if __name__ == "__main__":
    ...
    results = batch_compare_models(
        test_symbols, interval="4h", epochs=50, batch_size=32
    )
