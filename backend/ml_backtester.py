...
class MLTradingBacktester:
        ...
        for symbol in self.symbols:
            try:
                predictor = IntradayPredictor(
                    ...
                    lookback_periods=168 if self.interval == "1h" else 42  # 1 week of data
                )
    ...
    def fetch_historical_data(self) -> Dict[str, pd.DataFrame]:
        ...
        for symbol in self.symbols:
            try:
                ...
                data = yf.download(
                    symbol,
                    start=self.start_date - timedelta(days=10),  # Extra data for indicators
                    end=self.end_date,
                    interval=self.interval,
                    progress=False
                )
    ...
    def generate_trading_signal(
        ...
        try:
            ...
            return {
                ...
                'confidence': abs(predicted_change) / 10.0  # Normalize to 0-1 range
            }
    ...
    def calculate_position_size(
        ...
        if signal_data['signal'] == 'BUY':
            ...
            available_to_buy = min(self.cash * 0.9, max_position_value - current_position_value)
            return available_to_buy * confidence if available_to_buy > 0 else 0.0
            
        elif signal_data['signal'] == 'SELL':
            # Sell portion based on confidence
            return current_position_value * confidence if current_position_value > 0 else 0.0
    ...
    def _calculate_performance_metrics(self) -> Dict:
        ...
        winning_trades = sum(1 for t in self.trade_history if t['predicted_change'] * (1 if t['action'] == 'BUY' else -1) > 0)
        ...
        return {
            ...
            'trading_stats': {
                ...
                'win_rate': win_rate,
                'avg_trade_size': np.mean([t['value'] for t in self.trade_history]) if num_trades > 0 else 0,
            },
            'final_positions': {
                symbol: {
                    'coins': position,
                    'value': position * portfolio_df['portfolio_value'].iloc[-1]  # Approximation
                }
...
if __name__ == "__main__":
    ...
    for trade in backtester.trade_history[-10:]:
        print(f"  {trade['timestamp']}: {trade['action']} {trade['coins']:.6f} {trade['symbol']} @ ${trade['price']:.2f}")
