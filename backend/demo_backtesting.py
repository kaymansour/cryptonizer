...
def demo_rebalancing_comparison():
    ...
    if results:
        ...
        for freq, data in results.items():
            print(f"{freq:<10} ${data['final_value']:<11,.0f} {data['total_return']:<7.2f}% {data['sharpe_ratio']:<7.3f}")
...
def main():
    ...
    total = len([r for r in results if r[0] != "API Usage Examples"])  # API demo always "passes"
