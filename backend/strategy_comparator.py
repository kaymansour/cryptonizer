...
class StrategyComparator:
    ...
    def compare_strategies(self, 
                          ...
                          include_benchmarks: List[str] = None) -> Dict:
        """
        ...
        print(f"Comparing {len(include_benchmarks) + 1} strategies over period {self.start_date} to {self.end_date}")
        ...
        if 'btc_only' in include_benchmarks and 'BTC-USD' in self.symbols:
            ...
            btc_weights = {symbol: 1.0 if symbol == 'BTC-USD' else 0.0 for symbol in self.symbols}
            strategies['btc_only'] = self._backtest_strategy(
                btc_weights,
                "Bitcoin Only (Buy & Hold)",
                "never"  # Buy and hold
            )
        ...
        if 'eth_only' in include_benchmarks and 'ETH-USD' in self.symbols:
            ...
            eth_weights = {symbol: 1.0 if symbol == 'ETH-USD' else 0.0 for symbol in self.symbols}
            strategies['eth_only'] = self._backtest_strategy(
                eth_weights,
                "Ethereum Only (Buy & Hold)",
                "never"  # Buy and hold
            )
        
        # 5. 60/40 BTC/ETH Portfolio
        if 'btc_eth_60_40' in include_benchmarks and 'BTC-USD' in self.symbols and 'ETH-USD' in self.symbols:
            print("Running 60/40 BTC/ETH backtest...")
            btc_eth_weights = {symbol: 0.6 if symbol == 'BTC-USD' 
                              else 0.4 if symbol == 'ETH-USD' 
                              else 0.0 for symbol in self.symbols}
    ...
    def _calculate_market_cap_weights(self) -> Optional[Dict[str, float]]:
        ...
        try:
            ...
            available_caps = {symbol: approximate_market_caps.get(symbol, 1_000_000_000) 
                            for symbol in self.symbols}
            total_cap = sum(available_caps.values())
            
            weights = {symbol: cap / total_cap for symbol, cap in available_caps.items()}
...
def compare_with_benchmarks(symbols: List[str],
                           optimized_weights: Dict[str, float],
                           start_date: str = None,
                           end_date: str = None,
                           initial_investment: float = 100000,
...
def quick_comparison(symbols: List[str], 
                    optimized_weights: Dict[str, float],
...
if __name__ == "__main__":
    ...
    try:
        ...
        for rank_data in results['rankings']['total_return'][:3]:
            print(f"  {rank_data['rank']}. {rank_data['strategy_name']}: {rank_data['value']:.2f}%")
